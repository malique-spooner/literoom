...
class ReviewUiTests(unittest.TestCase):
    ...
    def test_search_compare_and_review_pages_render(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("Inspect imports", system_page.text)
            self.assertIn("Smoke-test ingest", system_page.text)
            self.assertIn("Run full ingest", system_page.text)
            self.assertIn("Recovery", system_page.text)
            self.assertIn("Factory reset", system_page.text)
